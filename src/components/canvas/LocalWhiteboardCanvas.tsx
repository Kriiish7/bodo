...
export function LocalWhiteboardCanvas({ boardId }: { boardId?: string }) {
		...
		async (file: File) => {
			try {
				...
				for (let pageNum = 1; pageNum <= pdf.numPages; pageNum++) {
					...
					const displayHeight =
						(viewport.height / viewport.width) * displayWidth;
	...
	const isPointOverPdf = useCallback(
	...
	const getSmartStrokeColor = useCallback(
	...
	const onPointerDown = (e: React.PointerEvent) => {
		...
		if (tool === "path" || tool === "highlight") {
			...
			["rectangle", "ellipse", "diamond", "arrow", "line"].includes(tool)
		) {
			setIsDrawing(true);
			const id = generateId();
			setElements((prev) => [
	...
	const onPointerMove = (e: React.PointerEvent) => {
		...
		const coalescedEvents = (e.nativeEvent as any).getCoalescedEvents?.() as
			| PointerEvent[]
			| undefined;
		...
		setElements((prev) =>
			prev.map((el) => {
				...
				if (
					(tool === "path" || tool === "highlight") &&
					(el.type === "path" || el.type === "highlight")
				) {
	...
	const renderShapes = (renderElements = elements) => {
		return renderElements.map((el) => {
			...
			const stroke =
				isSelected && tool === "select" ? "#3b82f6" : el.stroke || "#000";
			...
			const commonProps = {
				...
				style: {
					pointerEvents:
						tool === "hand" && !isSelected ? "none" : "visiblePainted",
			...
			if (el.type === "path" || el.type === "highlight") {
				...
				const outlinePath = getStrokeOutline(
					el.points,
					el.pressures,
					el.x,
					el.y,
				...
				return (
					<path
						...
						d={d}
						{...commonProps}
						strokeOpacity={el.type === "highlight" ? 0.3 : 1}
						fill="none"
						style={{
							...(commonProps.style as any),
							filter: `drop-shadow(0px 0px ${sw}px ${stroke}aa)`,
						...
						>
							<textarea
								...
								onChange={(ev) => {
									...
									setElements((prev) =>
										prev.map((e) => (e._id === el._id ? { ...e, text } : e)),
									);
								}}
								...
								style={{
									...
									border:
										activeId === el._id && tool === "select" && !isSticky
											? "1px dashed #3b82f6"
											: "none",
		...
		>
			...
			<div className="absolute top-4 z-10 flex flex-col items-center gap-3">
					...
					{TOOLS.map((t, idx) => {
						if (t.divider) {
							return (
								<div
									key={`div-${idx}`}
									className="w-[1.5px] h-6 bg-slate-100 dark:bg-slate-600 mx-1"
								/>
							);
						}
						const Icon = t.icon!;
				...
				<div className="text-[12px] text-slate-400/90 dark:text-slate-500/90 font-medium select-none pointer-events-none tracking-wide text-center bg-white/80 dark:bg-[#2a2a2a]/80 backdrop-blur-sm rounded-full px-3 py-1 border border-slate-100/50 dark:border-slate-700/50 shadow-sm">
					To shift orbital view, hold{" "}
					<kbd className="px-1.5 py-[1px] bg-slate-100 dark:bg-slate-700 border border-slate-200/80 dark:border-slate-600 rounded text-slate-500/90 dark:text-slate-400 font-mono text-[10px]">
						Scroll wheel
					</kbd>{" "}
					or{" "}
					<kbd className="px-1.5 py-[1px] bg-slate-100 dark:bg-slate-700 border border-slate-200/80 dark:border-slate-600 rounded text-slate-500/90 dark:text-slate-400 font-mono text-[10px]">
					...
					while dragging, or use the tether
				</div>
			</div>
			...
			<div className="absolute bottom-4 left-4 z-10 flex items-center gap-2 pointer-events-auto">
				...
				<div className="flex border border-slate-200 dark:border-slate-700 rounded-lg shadow-sm p-1 bg-white dark:bg-[#2a2a2a] items-center gap-1 backdrop-blur-md">
					<Button
						...
						onClick={() =>
							setCamera((c) => ({ ...c, zoom: Math.max(c.zoom - 0.1, 0.1) }))
						}
					>
						-
					</Button>
					...
					<Button
						...
						className="h-7 w-7 rounded-sm text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white"
						onClick={() =>
							setCamera((c) => ({ ...c, zoom: Math.min(c.zoom + 0.1, 5) }))
						}
					>
						+
					</Button>
