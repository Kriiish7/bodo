...
import { ChevronRight, ChevronDown, Plus, Trash2, Link2 } from "lucide-react";
...
export function TableNode({ el, isActive, setElements, allElements }: TableNodeProps) {
...
<React.Fragment key={row.id}>
				<div 
                    className="flex items-center group border-b border-zinc-200 dark:border-white/10 hover:bg-black/5 dark:hover:bg-white/5 transition-colors"
                >
                    <div className="flex-1 flex items-center p-1" style={{ paddingLeft: `${depth * 1.5 + 0.5}rem` }}>
                        <button
                        ...
                        {row.type === "relation" ? (
                            <div className="flex items-center px-2 py-1 gap-2 flex-1 text-blue-500 bg-blue-500/10 rounded mr-2 text-sm">
                                ...
                                <select 
                                    ...
                                    onChange={(e) => handleUpdate("relationTarget", e.target.value)}
                                >
                                    <option value="">Select target table...</option>
                                    {allElements.filter(e => e.type === "table" && e._id !== el._id).map(t => (
                                        <option key={t._id} value={t._id}>{t.title || 'Untitled Table'}</option>
                                    ))}
                                </select>
                            </div>
                        ) : (
                    ...
                    <div className="flex items-center justify-center p-1 gap-1 border-l border-zinc-200 dark:border-white/10 opacity-0 group-hover:opacity-100 transition-opacity w-16">
                        <button onClick={handleAddChild} className="p-1.5 hover:bg-black/10 dark:hover:bg-white/10 rounded text-zinc-500">
                            <Plus size={14} />
                        </button>
                        <button onClick={handleDelete} className="p-1.5 hover:bg-red-500/20 rounded text-red-500">
                            <Trash2 size={14} />
                        </button>
                    </div>
				</div>
                {row.expanded && row.children && row.children.map(child => renderRow(child, depth + 1, [...parentIds, row.id]))}
			</React.Fragment>
		);
	};
...
isActive ? "ring-2 ring-blue-500 shadow-xl" : "shadow-md border border-zinc-200 dark:border-white/10"
}`}
		>
            {/* Header */}
            <div className="bg-zinc-50 dark:bg-zinc-800/80 p-3 flex items-center gap-3 border-b border-zinc-200 dark:border-white/10">
                <input
                    ...
                    onChange={(e) => {
                        setElements((prev) => prev.map((item) => (item._id === el._id ? { ...item, title: e.target.value } : item)));
                    }}
            ...
            <div className="flex text-xs font-semibold text-zinc-500 dark:text-zinc-400 uppercase tracking-wider border-b border-zinc-200 dark:border-white/10">
                ...
                <div className="flex-1 p-2 pl-3 border-l border-zinc-200 dark:border-white/10">Value</div>
                <div className="w-16 border-l border-zinc-200 dark:border-white/10"></div>
            </div>
            {/* Rows */}
            <div className="flex-1 overflow-y-auto min-h-[100px]">
                {defaultData.length === 0 ? (
                    <div className="h-full flex items-center justify-center text-sm text-zinc-400 italic">No data yet.</div>
                ) : (
