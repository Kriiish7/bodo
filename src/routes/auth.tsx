...
function Auth() {
    ...
    try {
      if (isLogin) {
        ...
        if (!res.error) {
          ...
          setErrorText(res.error.message || 'Failed to sign in. Please check your credentials.')
        }
      } else {
        if (!name) {
        ...
        if (!res.error) {
          ...
          setErrorText(res.error.message || 'Failed to create account. Please try again.')
        }
      }
    } catch (err: any) {
  ...
  const handleOAuthLogin = async (provider: 'github' | 'google') => {
    ...
    try {
      ...
      if (res.error) {
        setErrorText(res.error.message || `Failed to authenticate with ${provider}.`)
      }
    } catch (err: any) {
      setErrorText(err?.message || 'An unexpected error occurred.')
    } finally {
  ...
  return (
    <div className="flex h-screen w-full font-sans bg-zinc-100 overflow-hidden selection:bg-red-500/30">
      ...
      <div className="hidden lg:flex w-[45%] bg-[#0a0a0a] text-white flex-col justify-between p-12 relative overflow-hidden shadow-2xl z-10">
        ...
        {/* Dynamic Glow Orbs */}
        <div className="absolute top-[-10%] left-[-10%] w-[500px] h-[500px] rounded-full bg-red-900/20 blur-[100px] animate-pulse" style={{ animationDuration: '4s' }} />
        <div className="absolute bottom-[-10%] right-[-10%] w-[600px] h-[600px] rounded-full bg-red-600/10 blur-[120px] animate-pulse" style={{ animationDuration: '6s', animationDelay: '1s' }} />
        ...
        <div className="relative z-20 flex flex-col h-full">
          ...
          <div className="mb-20">
            ...
            <p className="text-lg text-zinc-400 max-w-md mb-12 leading-relaxed">
              Experience the friction-less infinite canvas built for the modern team. Ideate, map, and collaborate in real-time.
            </p>
            ...
            <div className="space-y-4">
              {[
                ...
                <div key={i} className="flex items-center gap-4 text-zinc-300 bg-white/4 p-4 rounded-2xl border border-white/6 backdrop-blur-sm transition-transform hover:translate-x-1">
                  <div className="h-8 w-8 rounded-full bg-red-500/15 flex items-center justify-center shrink-0">
                    <CheckCircle2 className="h-4 w-4 text-red-400" />
                  </div>
                  <span className="font-medium text-sm lg:text-base tracking-wide">{feature}</span>
                </div>
              ))}
            </div>
          </div>
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-5">
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between">
                ...
                {isLogin && (
                  <a href="#" className="text-xs font-semibold text-red-600 hover:text-red-500 transition-colors bg-red-50 px-2 py-1 rounded-md">
                    Forgot password?
                  </a>
                )}
              </div>
            ...
            >
              {isLoading ? (
                ...
                <>Create Account <ArrowRight className="ml-2 h-4 w-4 opacity-70" /></>
              )}
            </Button>
          </form>
