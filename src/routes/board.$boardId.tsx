...
function Board() {
  ...
  if (board === undefined || session.isPending) {
    return (
  ...
  if (board === null) {
    return (
      <div className="fixed inset-0 z-50 flex flex-col h-screen items-center justify-center p-4 bg-[#0a0a0a]">
        ...
        <p className="text-zinc-500 mb-6">This board may have been deleted or doesn't exist.</p>
        <Button onClick={handleBack} className="bg-red-600 hover:bg-red-500 text-white rounded-full px-6 border border-red-500/30">
          Return to Dashboard
        </Button>
      </div>
    )
    ...
    <div className="fixed inset-0 z-50 flex w-full flex-col bg-[#fdfdfd] dark:bg-[#0a0a0a] overflow-hidden">
      ...
      <header className="absolute top-4 left-4 right-4 z-10 flex items-center justify-between rounded-xl border border-zinc-200 dark:border-white/6 bg-white/90 dark:bg-zinc-900/90 p-3 shadow-sm backdrop-blur-lg">
        <div className="flex items-center gap-3">
          <Button variant="ghost" size="icon" onClick={handleBack} className="text-zinc-500 hover:text-zinc-800 dark:hover:text-white rounded-lg h-8 w-8">
            <ArrowLeft className="h-4 w-4" />
          </Button>
          <div className="h-5 w-px bg-zinc-200 dark:bg-white/10" />
          <h1 className="text-sm font-semibold text-zinc-800 dark:text-white px-1 truncate max-w-[200px]">{board.title}</h1>
