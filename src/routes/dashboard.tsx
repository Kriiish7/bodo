...
function Dashboard() {
	...
	const filteredBoards = boards?.filter((b: any) => activeSpaceId ? b.spaceId === activeSpaceId : true);
					...
					<div>
						...
						<div className="flex flex-col gap-1">
							...
							{spaces?.map((space: any) => (
								<div key={space._id} className="group relative flex items-center">
									<button 
									...
									>
										<FolderOpen className="w-4 h-4" /> <span className="truncate">{space.name}</span>
									</button>
									<button
										onClick={(e) => {
											e.stopPropagation();
				...
				<main className="flex-1 p-6 md:p-10 w-full overflow-y-auto">
					...
					<div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-6">
						<div>
							<h2 className="text-3xl font-bold text-white tracking-tight mb-1">
								{activeSpaceId ? spaces?.find((s: any) => s._id === activeSpaceId)?.name : `Welcome back, ${userName}`}
							</h2>
							<p className="text-zinc-500 text-sm">
								{filteredBoards?.length || 0} board{filteredBoards?.length !== 1 ? "s" : ""} in
					...
					<div className="mb-10">
						...
						<div className="grid grid-cols-2 md:grid-cols-4 gap-4">
							<button 
								onClick={() => handleCreateBlueprint("Brainstorming", [{_id:"bs1",type:"sticky",x:-100,y:-50,width:200,height:150,text:"Big Idea 1!",fill:"#fef3c7"}])} 
							...
							>
								...
								<span className="font-medium text-sm text-blue-100">Brainstorming</span>
							</button>
							<button 
								onClick={() => handleCreateBlueprint("Flowchart", [{_id:"fc1",type:"rectangle",x:-150,y:0,width:100,height:50,stroke:"#ffffff"},{_id:"fc2",type:"arrow",x:-50,y:25,width:50,height:0,stroke:"#ffffff"},{_id:"fc3",type:"diamond",x:0,y:0,width:80,height:50,stroke:"#ffffff"}])} 
							...
							>
								...
								<span className="font-medium text-sm text-emerald-100">Flowchart</span>
							</button>
							<button 
								onClick={() => handleCreateBlueprint("Daily Standup", [{_id:"ds1",type:"text",x:-200,y:-100,width:150,height:40,text:"Yesterday",stroke:"#ffffff"},{_id:"ds2",type:"text",x:0,y:-100,width:150,height:40,text:"Today",stroke:"#ffffff"},{_id:"ds3",type:"text",x:200,y:-100,width:150,height:40,text:"Blockers",stroke:"#ffffff"}])} 
							...
							>
								...
								<span className="font-medium text-sm text-purple-100">Daily Standup</span>
							</button>
							<button 
							...
							>
								...
								<span className="font-medium text-sm text-zinc-300">Blank Canvas</span>
							</button>
						</div>
					</div>
				...
				<DialogContent className="sm:max-w-[425px] bg-[#0a0a0a] border-white/10 text-white">
					<DialogHeader>
						...
						<DialogDescription className="text-zinc-400">
							Spaces help you organize boards into projects. Give your space a name.
						</DialogDescription>
					</DialogHeader>
					<form onSubmit={handleCreateSpace} className="flex flex-col gap-4 mt-4">
						<Input
							autoFocus
						...
						<DialogFooter>
							<div className="flex justify-end gap-2 w-full">
								<Button type="button" variant="ghost" className="hover:bg-white/5 hover:text-white" onClick={() => setIsSpaceDialogOpen(false)}>Cancel</Button>
								<Button type="submit" className="bg-red-600 hover:bg-red-500 text-white border-red-500/30">Create Space</Button>
							</div>
						</DialogFooter>
					</form>
				</DialogContent>
