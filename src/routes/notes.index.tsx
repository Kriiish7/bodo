  ...
  return (
    <div className="max-w-4xl mx-auto p-8 flex flex-col gap-8">
      <div className="flex justify-between items-center bg-white p-6 rounded-2xl shadow-sm border border-zinc-100">
        <div>
          <h1 className="text-3xl font-bold bg-clip-text text-transparent bg-linear-to-r from-zinc-800 to-zinc-500">Collaborative Notes</h1>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        {notes?.length === 0 ? (
          <div className="col-span-full flex flex-col items-center justify-center p-16 border-2 border-dashed border-zinc-200 rounded-2xl bg-zinc-50/50">
            <h3 className="font-semibold text-zinc-900 text-lg">No notes found</h3>
          ...
          notes?.map((note) => (
            ...
            >
              ...
              <h3 className="font-semibold text-lg text-zinc-800 truncate">{note.title || 'Untitled'}</h3>
              <p className="text-zinc-500 text-sm truncate flex-1">{note.content || 'Empty note'}</p>
